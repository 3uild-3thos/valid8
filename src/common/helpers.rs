use std::{fs::{File, create_dir_all}, path::Path, io::{Write, Read}};
...
pub fn create_resources_dir() -> Result<()> {
    create_dir_all(Path::new(".valid8/"))?;
    Ok(())
}

pub fn create_new_workspace() -> Result<()> {
    File::create(Path::new("./valid8.json"))?;
    Ok(())
}
...
pub fn fetch_account(network: &Network, pubkey: &Pubkey) -> Result<AccountSchema> {
    ...
    Ok(AccountSchema::from_account(pubkey, &account, network))
}
...
pub fn save_idl(pubkey: &Pubkey, data: &Vec<u8>) -> Result<()> {
    let mut f = File::create(Path::new(&format!("./.valid8/{}.idl.json", pubkey)))?;
    f.write_all(&data)?;
    Ok(())
}
